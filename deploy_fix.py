...
commands = [
    "sudo git config --global --add safe.directory /home/ubuntu/OrbeSystems",
    "cd /home/ubuntu/OrbeSystems",
    "sudo git config --global --add safe.directory /home/ubuntu/OrbeSystems",
    "sudo git pull origin main --rebase || echo 'Git pull failed'",
    "sudo docker-compose -f ec2_compose.yml stop inho_backend",
    "sudo docker-compose -f ec2_compose.yml build inho_backend",
    "sudo docker-compose -f ec2_compose.yml up -d inho_backend"
]
...
print("Triggered deploy:", command_id)
...
while True:
    ...
    with open("deploy_fix_out.txt", "w", encoding="utf-8") as f:
        f.write(out_text)
    print("SAVED TO deploy_fix_out.txt")
    break
