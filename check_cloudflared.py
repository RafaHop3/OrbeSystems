...
payload = {
    ...
    "Parameters": {
        "commands": [
            "sudo systemctl status cloudflared || echo 'NOT FOUND'",
            "sudo sed -i 's/\"80:8000\"/\"8000:8000\"/g' /home/ubuntu/orbe-systems/docker-compose.yml",
            "cd /home/ubuntu/orbe-systems && sudo docker compose down",
            "cd /home/ubuntu/orbe-systems && sudo docker compose up -d"
        ]
    }
}
with open("ssm_cloudflared.json", "w") as f: json.dump(payload, f)
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://ssm_cloudflared.json", "--region", "us-east-1", "--output", "json"], text=True)
