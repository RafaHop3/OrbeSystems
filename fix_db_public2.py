...
payload = {
    ...
    "Parameters": {
        "commands": [
            ...
            "cat << 'EOF' > fix_schema_only_public.py\n" + py_script.replace('$', '\\$') + "\nEOF",
            "sudo docker compose -f /home/ubuntu/orbe-systems/docker-compose.yml cp fix_schema_only_public.py inho_backend:/app/fix_schema_only_public.py",
            "sudo docker compose -f /home/ubuntu/orbe-systems/docker-compose.yml exec -T inho_backend /opt/venv/bin/python /app/fix_schema_only_public.py"
        ]
    }
}

with open("fix_db_schema_public2.json", "w") as f:
    json.dump(payload, f)

res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://fix_db_schema_public2.json", "--region", "us-east-1", "--output", "json"])
