...
payload = {
    ...
    "Parameters": {
        "commands": [
            "sudo docker rm -f orbe_postgres orbe_backend orbe_redis || true",
            "cd /home/ubuntu/orbe-systems && sudo docker compose up -d",
            "sudo docker ps"
        ]
    }
}
with open("ssm_nuke_recreate.json", "w") as f: json.dump(payload, f)
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://ssm_nuke_recreate.json", "--region", "us-east-1", "--output", "json"], text=True)
