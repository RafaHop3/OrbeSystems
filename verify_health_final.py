...
with open("verify_final_req.json", "w") as f: json.dump(payload, f)

res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://verify_final_req.json", "--region", "us-east-1", "--output", "json"])
...
for _ in range(12):
    ...
    if out.returncode == 0:
        ...
        if data.get("Status") in ["Success", "Failed"]:
            with open("verify_final_out.txt", "w", encoding="utf-8") as lf:
