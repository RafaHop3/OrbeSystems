...
payload = {
    ...
    "Parameters": {
        "commands": [
            ...
            "sudo docker compose up -d --force-recreate orbe_backend"
        ]
    }
}
with open("fix_env_req.json", "w") as f: json.dump(payload, f)

res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://fix_env_req.json", "--region", "us-east-1", "--output", "json"])
