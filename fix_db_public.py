...
payload = {
    ...
    "Parameters": {
        "commands": [
            ...
            "sudo docker compose -f /home/ubuntu/orbe-systems/docker-compose.yml exec -T inho_backend python /app/fix_schema_only_public.py"
        ]
    }
}

with open("fix_db_schema_public.json", "w") as f:
    json.dump(payload, f)

res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://fix_db_schema_public.json", "--region", "us-east-1", "--output", "json"])
