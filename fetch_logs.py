...
payload = {
    ...
    "Parameters": {
        "commands": ["sudo docker logs --tail 30 orbe_backend"]
    }
}
with open("ssm_get_logs.json", "w") as f: json.dump(payload, f)
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://ssm_get_logs.json", "--region", "us-east-1", "--output", "json"], text=True)
