                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
                                <Users className="text-teal-400" /> Carteira de Clientes
                            </h1>
                            <p className="text-[#8b949e] font-mono text-sm max-w-2xl leading-relaxed">
                                Gestão centralizada de clientes e prospectos. Adicione novas entidades para originar faturas ou contratos de recorrência no master DB.
                            </p>
                        </div>
                        <button
                    ...
                    <div className="bg-[#0A1820]/50 border border-teal-500/20 rounded-xl flex-1 flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-teal-500/20 flex gap-4">
                            <div className="flex-1 relative">
                                ...
                                <input type="text" placeholder="Buscar por nome, documento ou e-mail..." className="w-full bg-[#12222A] border border-teal-500/30 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-teal-400 text-white" />
                            </div>
                        </div>

                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 opacity-60">
                            <Users size={48} className="text-[#1a2f3a] mb-4" />
                            <p className="text-lg font-semibold text-white mb-1">Nenhum cliente cadastrado</p>
                            <p className="text-[#8b949e] text-sm">Clique em "Novo Cliente" acima para realizar o onboarding fiscal B2B2C.</p>
                        </div>
                    </div>
                </div>
            </main>

            {showModal && <ModalNovoSocio onClose={() => setShowModal(false)} category="CUSTOMER" />}
        </div>
    );
}
