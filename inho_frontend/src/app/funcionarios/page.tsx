                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
                                <UserCheck className="text-teal-400" /> Recursos Humanos (Funcionários)
                            </h1>
                            <p className="text-[#8b949e] font-mono text-sm max-w-2xl leading-relaxed">
                                Cadastro central de colaboradores. Use este painel para registrar CLT/PJ para viabilizar pagamentos de folha no financeiro.
                            </p>
                        </div>
                        <button
                    ...
                    <div className="bg-[#0A1820]/50 border border-teal-500/20 rounded-xl flex-1 flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-teal-500/20 flex gap-4">
                            <div className="flex-1 relative">
                                ...
                                <input type="text" placeholder="Buscar por Nome, CPF ou Departamento..." className="w-full bg-[#12222A] border border-teal-500/30 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-teal-400 text-white" />
                            </div>
                        </div>

                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 opacity-60">
                            <UserCheck size={48} className="text-[#1a2f3a] mb-4" />
                            <p className="text-lg font-semibold text-white mb-1">Quadro letargico vazio</p>
                            <p className="text-[#8b949e] text-sm">Registre colaboradores para permitir pagamentos centralizados no Master DB.</p>
                        </div>
                    </div>
                </div>
            </main>

            {showModal && <ModalNovoSocio onClose={() => setShowModal(false)} category="EMPLOYEE" />}
        </div>
    );
}
