...
payload = {
    ...
    "Parameters": {
        "commands": [
            "sudo docker logs --tail 200 orbe_backend 2>&1 | grep -iE 'error|exception|traceback|fail'"
        ]
    }
}
with open("ssm_worker_errors.json", "w") as f: json.dump(payload, f)
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://ssm_worker_errors.json", "--region", "us-east-1", "--output", "json"], text=True)
