...
with open("deploy_req.json", "w") as f:
    json.dump(payload, f)
...
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://deploy_req.json", "--region", "us-east-1", "--output", "json"], env=env)
...
for _ in range(40):
    ...
    if out.returncode == 0:
        ...
        if data.get("Status") in ["Success", "Failed"]:
            ...
            print("Status:", status)
            with open("deploy_out.txt", "w", encoding="utf-8") as outf:
