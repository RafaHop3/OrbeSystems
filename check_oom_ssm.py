...
commands = [
    "sudo docker inspect inho_backend -f 'OOMKilled: {{.State.OOMKilled}} | ExitCode: {{.State.ExitCode}} | Status: {{.State.Status}}'",
    "free -m"
]
...
while True:
    ...
    with open("oom_check.txt", "w", encoding="utf-8") as f:
        f.write(out_text)
    print("SAVED TO oom_check.txt")
    break
