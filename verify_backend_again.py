...
payload = {
    ...
    "Parameters": {
        "commands": [
            "sudo docker ps -a",
            "sudo docker logs --tail 30 orbe_backend 2>&1"
        ]
    }
}
with open("ssm_verify_backend.json", "w") as f: json.dump(payload, f)
res = subprocess.check_output(["aws", "ssm", "send-command", "--cli-input-json", "file://ssm_verify_backend.json", "--region", "us-east-1", "--output", "json"], text=True)
