...
commands = [
    "sudo docker logs orbe_backend --tail 50 || echo 'NO LOGS'",
    "echo '---- INHO-API NGINX FILE ----'",
    "cat /etc/nginx/sites-available/inho-api || echo 'NO INHO NGINX'"
]
...
while True:
    ...
    with open("docker_crash_out.txt", "w", encoding="utf-8") as f:
        f.write(out_text)
    print("SAVED TO docker_crash_out.txt")
    break
